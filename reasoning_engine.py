...
v1.0.5: 修复 text_chat 调用签名（统一 ProviderRequest）
"""
...
class ReasoningEngine:
    ...
    async def build_plan(self, event: AstrMessageEvent, question: str,
        ...
        pos, neg = await self.memory_mgr.retrieve_balanced_memories(
            query=question, pos_top_k=2, neg_top_k=2,
        )
    ...
    async def judge_replan(self, event: AstrMessageEvent, execution_trace: str) -> str:
        system_prompt = (
            "You are a critical evaluation assistant. Analyze the execution trace "
            "and determine if replanning is needed."
        )
        user_prompt = f"## 执行轨迹\n{execution_trace}\n\n## 输出要求\n请只输出 'yes' 或 'no'"
    ...
    async def build_replan(self, event: AstrMessageEvent, question: str,
        ...
        pos, neg = await self.memory_mgr.retrieve_balanced_memories(
            query=question, pos_top_k=4, neg_top_k=4,
        )
        memories_text = self._format_memories_for_prompt(pos, neg)
        system_prompt = (
            "You are a strategic replanning assistant. "
