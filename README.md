...
### v2.5.4 — 事件驱动进化 + 蒸馏鲁棒性 (2026-05-14)
- 🔄 **事件驱动进化**: `_evolution_loop` 重写 — 50轮阈值 + 2h空闲超时双触发，替代硬编码 6h 定时器。`on_llm_request` 累加计数器，无对话不触发，天然规避深夜 API TLS 挂死后空转
- 🔬 **跨类型蒸馏**: `distill_rules()` 候选池从 declarative+consolidated_rule → 四类全包 (declarative + consolidated_rule + procedural + insight)，消除「分类即命运」
- 🧠 **Prompt 抽象约束**: LLM 被要求以方法论为主体框架，操作级记忆作为示例融入，防止多条并列规则
- 🧹 **存储前语义去重**: 新规则 cosine sim >= 0.85 与已有 DR-* 合并而非新建，实现「压缩」而非「复制」
- 🎯 **Provider 优先级**: deepseek 官方 > opencode-go > get_using_provider() > providers[0]，避免 opencode-go TLS 挂死
- ⏰ **蒸馏窗口放宽**: 2-6h → 2-8h，加 DEBUG 日志覆盖（入口/候选池/provider 类型）
- 🚫 **DISABLE_AUTO_EVOLUTION = False**: 保护充分，恢复自动进化
- 📝 **CLAUDE.md 同步**: 版本号 + 新特性 + Forgejo 推送备忘

### v2.5.3 — Near-miss 去重日志 (2026-05-14)
- 🔍 **Near-miss 去重日志**: 入库余弦相似度 0.78<sim<0.85 的「差一点去重」记录到日志，方便调参观察到去重阈值是否合理

### v2.5.1 — 基础设施硬化 (2026-05-14)
- 🔗 **SQLite 连接复用**: `_get_conn()` 懒连接 + `SELECT 1` 健康检查 + `wal_autocheckpoint`，17 处 connect/close → 1 个长连接
- 📊 **向量矩阵缓存**: `_get_cached_matrix()` 懒重建 + dirty flag，避免每次检索重建 60MB 矩阵
- 🛡️ **`/getoggle` 权限检查**: 配置驱动 `admin_whitelist` + 审计日志，未授权拦截
- 🏥 **健康检查清扫**: `_health_check_loop` 追加 `_walkman_cleanup_stale()`，定期清理残留状态
- 📐 **win_rate 公式修正**: `(s+1)/(s+2)` → `(s+1)/(s+f+2)`，failure_count 真正参与计算
- 🗃️ **Migration v2**: 新增 `failure_count` 列（INTEGER DEFAULT 0）

### v2.5.0 — Shadow Challenger Protocol (2026-05-13)
- 🥷 **Shadow Challenger (ChallengerModule)**: 双阈值触发（熵<0.15 或 10 轮无挑战）→ 从 Top-3 exploit 中随机选 victim → 用 cold/explore 挑战者替换 → 差分验证。不伪造失败，让真实反馈决定老兵去留
- 📊 `/gec` 命令查看挑战日志（victim WR / challenger WR / entropy）
